	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				...
				} elseif($uitype == 'FILEFLD') {
					foreach($_FILES as $fileindex => $files)
					{
						if($files['name'] != '' && $files['size'] > 0 && $fileindex == $fieldname)
						{
							$files['original_name'] = vtlib_purify($_REQUEST[$fileindex.'_hidden']);
							$fileid = $this->saveFileField($this->id,$module,$files);

							if(isset($fileid) && $fileid!=0 && $fileid!='')
								$this->column_fields[$fieldname] = $fileid;
						}
					}
...
//Added by Richie
	...
	/**
	 * Pavani - Jetweb (Pty) Ltd.
	 * Function to upload and add attachment
	 * @param <Int> $id - recordid
	 * @param <String> $module
	 * @param <Array> $file_details
	 * @return Integer - attachmentid
	 * Added new function to not dusturb uploadAndSaveFile()
	 */
    function saveFileField($id,$module,$file_details)
	{
		global $log;
